...
- **`/enquiry`'s 0.097 was the empty-basket state arriving at hydration** and was fixed the same day (`EnquiryForm.tsx`, the `ready` comment): 0 on phone and desktop since. A returning buyer with a saved basket shifted 0.18 on a phone — the list arrives at hydration above the form, and the server cannot know its height — until the same day, when the head script began publishing the line count so the page reserves the list's height before paint. 0 at every width measured since.
