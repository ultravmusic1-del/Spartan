...
- **Tasks 5 through 11: done and committed. The plan is complete**, at 17/17 gates, 310 unit tests and 287 end-to-end tests.
- **Task 6 corrections.** `DataState` takes `state` and `subject` and is not a wrapper with children, so the drafted `<DataState result={...}>` around the whole list would not have compiled and would have hidden the categories behind a failed product read. `Notice` already renders nothing for a null notice, so the `{notice && }` guard is dead. The two reads fail independently. Catalogue was added to the admin nav, which the plan never mentions — without it the six routes are reachable only by typing a URL.
- **Task 7 correction: the "Add a specification" button is gone.** As drafted it submitted the form exactly as Save did, and the page appended one blank row either way — a control that does nothing, which is the defect this repo has already removed twice. Three blank rows do the job with no JavaScript and no query parameter.
- **A defect neither the plan nor Task 4 anticipated: absent is not blank.** A key missing from a FormData meant `''` everywhere in the write side, which made every partial POST destructive — and the plan's own end-to-end test posts three fields, so it would have deleted every specification on the product it was proving could not be hijacked. Fixed uniformly in `given()`. The two fields a forged POST could still blank were closed by tightening the SHARED schema (`.min(1)` on `productSchema.name`, `productSchema.categoryId`, `categorySchema.name`, `categorySchema.description`) rather than by adding admin-side rules.
- **Task 10 correction: two forged-POST tests passed against a 403.** Astro's `security.checkOrigin` refuses an on-demand POST with no matching Origin header, so both were asserting that a read-only field had not moved about a request the server never processed. `forge()` now requires a 302 to `notice=catalogue-saved` before it checks anything, and the origin defence has a test of its own.
- **Task 10, second correction: three enquiry tests hard-coded `recorded: false`.** That was true only because no run had ever had a database behind the preview server. `tests/e2e/stack.ts` derives the expected outcome from the run's configuration and both branches are asserted in full.
- **Task 11 addition.** `docs/CONTENT-EDITING.md` now says at the top that `/admin/catalogue` exists, and that production has read the catalogue from Postgres since 2026-08-19 — so a file edit made by following that guide changes nothing on the live site by itself.
...
**Known soft spots, flagged rather than hidden.** Task 1 rebuilds the schema from migration history, which captures only what was applied *through* migrations; Step 4 diffs the result against a real inventory and says what to do about anything missing, `enquiry_lines` especially. Task 2 Step 4 cannot know in advance whether `seed.sql` alone populates products, and says what to do either way. Task 6 assumes `DataState`'s prop is named `result` and says to check. Task 10's sign-in selectors depend on the login form's labels and say to adjust the selectors rather than the assertions. The two edit-result types are named `ProductEditResult` and `CategoryEditResult` rather than sharing one generic, because their success payloads differ and a shared type would need a discriminant nothing else uses.
