...
console.log(`resizing ${products.length} product images…`);
...
// The hero entry is the client's supplied brand artwork, not an extraction —
// it is a PNG where the division photographs are JPEGs, so the sheet's copy is
// re-encoded to .jpg and the source extension is dropped rather than carried
// through. Everything below indexes `heroes` by position, so the landscape
// artwork must stay first.
const heroes = [
  ['hero-range-desktop.png', 'Product range — the home page hero'],
...
/** The sheet's own copy of each hero, always JPEG whatever the source was. */
const sheetName = (file) => file.replace(/\.[^.]+$/, '.jpg');
for (const [file] of heroes) {
  await sharp(join(ROOT, 'src/assets/hero', file))
    .resize({ width: 1400, withoutEnlargement: true })
    .flatten({ background: '#000000' })
    .jpeg({ quality: 88, mozjpeg: true })
    .toFile(join(WORK, 'h', sheetName(file)));
}
...
<section class="page">
  ...
  <div class="heroes">
    <figure class="hero-wide">
      <img src="${url(`h/${sheetName(heroes[0][0])}`)}">
      <figcaption>${heroes[0][1]} — 1672×941, supplied brand artwork</figcaption>
    </figure>
    ${heroes
      .slice(1)
      .map(
        ([f, cap]) =>
          `<figure><img src="${url(`h/${sheetName(f)}`)}"><figcaption>${cap}</figcaption></figure>`,
      )
      .join('')}
  </div>
</section>
